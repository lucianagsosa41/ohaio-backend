...
exports.summary = async (req, res) => {
  ...
  try {
    ...
    const r = await pool.query(q, [
      from, to, fromPrev.toISOString().slice(0,10), toPrev.toISOString().slice(0,10)
    ]);
    res.json({
      scope, from, to,
      ingresos: Number(r.rows[0].bruto),
...
exports.ivaBreakdown = async (req, res) => {
  ...
  try {
    ...
    res.json(r.rows.map(x => ({
      iva_pct: Number(x.iva_pct), neto: Number(x.neto), iva: Number(x.iva), bruto: Number(x.bruto)
    })));
  } catch (e) {
...
exports.topProductos = async (req, res) => {
  ...
  try {
    ...
    res.json(r.rows.map(x => ({
      item: x.descripcion, unidades: Number(x.unidades), ventas: Number(x.ventas)
    })));
  } catch (e) {
